...
class PaymentService
{
     ...
     */
    public function getPaymentData($paymentKey, &$paymentRequestData, $payment_reference)
    {
	    ...
	    if(in_array($paymentKey, ['NOVALNET_CC', 'NOVALNET_SEPA', 'NOVALNET_PAYPAL', 'NOVALNET_INVOICE'])) {
		    ...
		    $paymentAction = $this->paymentHelper->getNovalnetConfig(strtolower($paymentKey) . '_payment_action');
		    if((is_numeric($onHoldLimit) && $paymentRequestData['amount'] >= $onHoldLimit && $paymentAction == 'true') || ($paymentAction == 'true' && empty($onHoldLimit))) {
	    ...
	    if($this->isRedirectPayment($paymentKey) && $payment_reference != 'true')
	    {
